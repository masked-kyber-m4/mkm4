...
uint16_t sk1[SABER_K][SABER_N], sk2[SABER_K][SABER_N]; 
...
static int test_keys(void) {
  ...
  // Alice uses Bobs response to get her secret key
	BS2POLVECq_masked(sk_a, sk2, sk1); 
  FILL_STACK()
  crypto_kem_dec_masked_inner(key_a, sendb, sk1, sk2, sk_a+ SABER_SECRETKEYBYTES-SABER_KEYBYTES, sk_a + SABER_SECRETKEYBYTES-64, sk_a + SABER_INDCPA_SECRETKEYBYTES);
  CHECK_STACK()
  if(c >= canary_size) return -1;
    stack_decaps = c;
